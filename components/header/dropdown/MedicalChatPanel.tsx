...
const introMessage: ChatMessage = {
  role: "assistant",
  content:
    "مرحبا، أنا مساعد ذكاء اصطناعي للإجابة عن الأسئلة الطبية فقط. قد تحتوي إجاباتي على أخطاء، لذلك من الأفضل التواصل مع مختص طبي حقيقي، خصوصا في الحالات العاجلة.",
};
  ...
  const sendMessage = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/medical-chat", {
        ...
        body: JSON.stringify({
          messages: nextMessages.filter((message) => message !== introMessage),
        }),
      });
